/**
 * Categories API – same contract as types: GET/POST, 503/400/500/200.
 */
import { describe, it, expect, vi, beforeEach } from 'vitest'

const { mockAdd, mockCollection, mockAdminDb } = (() => {
  const mockAdd = vi.fn().mockResolvedValue({ id: 'mock-id-123' })
  const mockDocs = [
    { id: '1', data: () => ({ name: 'UGC / TikTok Ads' }) },
    { id: '2', data: () => ({ name: 'Product / Photography' }) },
  ]
  const mockCollection = vi.fn(() => ({
    add: mockAdd,
    get: () => Promise.resolve({ docs: mockDocs }),
  }))
  return { mockAdd, mockCollection, mockAdminDb: { collection: mockCollection } }
})()
...
describe('categories API', () => {
  ...
  it('POST returns 200 with id when name is valid', async () => {
    ...
    const req = new Request('http://test/api/categories', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: 'Fashion / Models' }),
    })
    const res = await POST(req)
    const data = await res.json()
    expect(res.status).toBe(200)
    expect(data).toMatchObject({ success: true, name: 'Fashion / Models' })
