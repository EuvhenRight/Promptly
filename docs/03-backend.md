...
- **Server Actions** — окремі файли з `'use server'` (наприклад, `admin/prompts/actions.ts`, AI flow).
...
| POST | `/api/checkout/fulfill` | Після успішної оплати — оновлення Firestore (credits, purchasedPrompts, orders, purchaseHistory). |
...
- **`src/app/admin/prompts/actions.ts`** — дії адмінки для промптів (створення/редагування/видалення тощо).
...
- Логіка скрапінгу для імпорту промптів знаходиться в адмін-розділі (`admin/prompts/scraper.tsx` або подібне). Вона викликає потрібні API або Server Actions для збереження в Firestore.
