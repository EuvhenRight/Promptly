...
export function StripeCheckout({
	...
	const rightContent = () => {
		...
		if (isLoadingSecret || !clientSecret) {
			return (
				<div className='flex flex-col items-center justify-center gap-4 min-h-[360px] text-muted-foreground'>
	...
	return (
		<div className='w-full min-h-[80vh]'>
			<div className='grid grid-cols-1 lg:grid-cols-2 min-h-[80vh]'>
				{/* Left: half screen, theme-aware background */}
				<div className='flex flex-col bg-muted/50 p-8 lg:p-10 xl:p-12 border-r border-border'>
					<Link
						href={backHref}
					...
					<h1 className='font-headline text-lg font-bold text-foreground mb-1'>
						Promptly
					</h1>
					<p className='text-muted-foreground text-[15px] mb-8'>Purchase</p>

					{/* Subscription/title line like reference */}
					<p className='text-foreground/90 text-[15px] mb-4'>
						{productName}
					</p>
					{/* Large price */}
					{displayPrice != null && (
						<p className='text-3xl xl:text-4xl font-bold text-foreground leading-tight mb-1'>
							{displayPrice}
						</p>
					)}
					...
					<div className='flex gap-3 mb-6'>
						...
						{displayPrice != null && (
							<span className='text-[15px] font-medium text-foreground shrink-0'>
								{displayPrice}
							</span>
						)}
					</div>
					...
					<div className='space-y-3 pt-5 border-t border-border'>
						<div className='flex justify-between text-[15px]'>
							...
							{displayPrice != null && (
								<span className='text-foreground/90'>{displayPrice}</span>
							)}
						</div>
						<button
							type='button'
							className='w-full text-left text-[15px] text-muted-foreground py-3 px-4 rounded-lg bg-muted hover:bg-muted/80 border border-border mt-2 disabled:opacity-50'
							disabled
						>
							Enter promo code
						</button>
