...
export default function EditPromptPage() {
  ...
  useEffect(() => {
    ...
    const fetchPrompt = async () => {
      setIsLoading(true);
      try {
        const data = await getPromptWithContent(
          firestore,
          params.id as string
        );
        if (data) {
  ...
  const initialDataForForm = {
    ...promptData,
    // The form does not expect an 'image' property with a File object unless it's being changed.
    // So we don't pass one initially.
  };
