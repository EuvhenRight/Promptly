...
export default function PromptClientPage() {
	...
	// --- Effects ---
    useEffect(() => {
        // Scroll to the top of the page whenever the component mounts or the ID changes.
        window.scrollTo(0, 0);
    }, [params.id]);
	...
	// NOTE: The main skeleton is now in the parent page.tsx
	if (isLoading || !prompt) {
		return null
	}
    
    // This check is now redundant because the skeleton in page.tsx will be shown
                        ...
                        <div className='group relative w-full overflow-hidden rounded-lg border bg-muted'>
                            {promptImage && (
                                <Image
                                    ...
                                    priority
                                    loader={isFirebaseStorageUrl(promptImage) ? firebaseImageLoader : undefined}
                                />
                            )}
                            {promptImage && (
